...
namespace interpreter {
    ...
    const std::unordered_map<std::string, operators> operator_map = {{"&", operators::AND}, {"||", operators::OR}, {"(", operators::LEFT_PAR}, {")", operators::RIGHT_PAR}};
    const std::unordered_map<std::string, int> operation_precedence = {{"&", 1}, {"||", 1}};
    ...
    void infix_to_postfix(const std::vector<std::string> &infix, std::vector<std::string> &postfix);
