...
int main() {
    ...
    std::remove("..\\dat_files\\cs.dat");
    std::ofstream out("..\\dat_files\\cs.dat", std::ios::binary);
    ser.serialize_column(out, cs);
    out.close();

    column dc;
    std::ifstream in("..\\dat_files\\cs.dat", std::ios::binary);
    ser.deserialize_column(in, dc);
    in.close();
    dc.print_column();
