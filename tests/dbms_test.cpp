...
TEST(get_table_test, get_table) {
    ...
    table* test_table =
        new table("INT int_col PK,DOUBLE dob_col,CHAR char_col,STR str_col");
    test_table->update_table("6,1.3,B,hello world");
    GTEST_ASSERT_EQ(
        test_dbms.get_table("a name")->get_contents().at(0).get_entry(0),
        test_table->get_contents().at(0).get_entry(0));
}
